...
#
# 30.8.2018
# letzte Überarbeitung und Anpassung log-Ausgabe und mehr
#
# 18-12-2016
# v1.4  8.1.2018
...
echo "Statuscode für $server: $Status_code"

 # wenn ja = 200
 if [ $Status_code = "200" ]; then
	echo "Hole WX Daten von $server für $LK"
	# Die Datei $PFAD/temp.EDDT liegt jetzt als HTML lokal vor.
	curl -s  --max-time 10 "$server$serv_str" > $PFAD/temp.$LK
	# Extrahieren aus der HTML-Seite mit sed:
	msg=$(cat $PFAD/temp.$LK |  grep '<temp_c>' | sed 's/<temp_c>/temp /g' | sed 's/<\/temp_c>//g' | sed 's/^[ \t]*//' | cut -d. -f1 )
